...
for jj = 3:length(lPro)
    ...
    if(strcmp(train,'train'))
        if(exist([fZug '\' nomCompo '.zug'])~=0)
            ...
            disp(['Le fichier ' lPro(jj).name ' ne correspond pas à un fichier compo existant'])
        end
    else
...
for jj = 3:length(lPro)
    if(fichierPRO_OK(jj,1))
        ...
        %Permet de récupérer le nom des manoeuvre des loco
...
%Permet de récuperer le noms des manoeuvres des locos qui sont dans le fichier train
