function ConfigurationTestConvStandard(fsl,ftoc,fTD,sep)
%Permet de créer les fichiers configurations. Pour trainDy, un fichier
%configuration est créé pour chaque composition et chaque manoeuvre
%envisagé (dans le dossier pro)
%les fichiers configurations sont créés à partir des fichiers .zug, des
%bases de données loco et wagons, de bremsen.dat (sous dossier dat), et des
...
h0=waitbar(0,'Conversion des configurations (standards)');

manoeuvresStandardsCompletes =  {'emergencyS_30kmh','service_1S_30kmh'};
manoeuvresStandards = {'emergencyS','service_1S'};

Vinit=30;
nomVoie = 'VoieStandard';
tempsEchantillonage = 0.1;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Vérification que les fichiers pro correspondent bien à un fichier compo
%existant
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
nbConfigEtTest = 0;
fichier_OK = zeros(length(lZug),1);
disp('Vérification des fichiers configurations à créer')
disp(sprintf('\n'))
for jj = 3:length(lZug)
    %disp(['Verification : ' num2str(jj-2) ' sur ' num2str(length(lPro)-2)])
    if(not(isempty(strfind(lZug(jj).name,'.zug'))))
        if(isempty(strfind(lZug(jj).name,'exemple')))
            fichier_OK(jj)=1;
        end
    end
end
...
for (typeManStandard = 1:length(manoeuvresStandards))
indConfig = 0;
%Boucle sur les fichiers configuration
for jj = 3:length(lZug)
   if(fichier_OK(jj))
        indConfig=indConfig+1;
        manoeuvresStandards{typeManStandard};
        disp(['Conversion configuration ',manoeuvresStandards{typeManStandard},' : ',num2str(indConfig),' sur ',num2str(sum(fichier_OK))]);
        waitbar(indConfig/sum(fichier_OK),h0);
        
        
        %Permet d'extraire le nom de la config et le nom du fichier train à
        %partir du titre du fichier pro
        nomCompo = lZug(jj).name(1:end-4);
        nomManTrain=manoeuvresStandardsCompletes{typeManStandard};
        
        %nom du fichier Config à ecrire
        nomConfig = [nomCompo '-' nomManTrain '.txt'];
        %Chemin du fichier config à ecrire
        nConfig = [fConfiguration fsl nomConfig];
        
        %Nom du fichier Test à ecrire
        nomTest = ['Test' '-' nomCompo '-' nomManTrain '.txt'];
        %Chemin du fichier Test à ecrire
        ...
        %Recupération des ligne locomotive du fichier zug
        trova_info('Loks:',nfZug);
        line = fgetl(nfZug);
        indice = 1;
        while(indice<=nombreLoco)
            if(not(isempty(line)))
                %Récupération et traitement des lignes de paramètre loco
        ...
        %Récupération des lignes wagons du fichier zug
        ...
        while(indice<=nombreWagon)
            %Vérification que la ligne n'est pas vide ou qu'elle ne
        ...
        for (ii = 1:nombreVehicule)
            ...
            if(estLoco(ii))
                ...
                if(indiceLoco==1)
                    matManoeuvre(ii)=cellstr([manoeuvresStandards{typeManStandard} 'Active']);
                else
                    matManoeuvre(ii)=cellstr([manoeuvresStandards{typeManStandard} 'Passive']);
                end
            else
        ...
        for (ii = 1:nombreVehicule)
            ...
            %Affichage du regime de freinage (G ou P) à titre indicatif juste
            %pour affichage
            ...
            %temps de montée 100% = temps 95 +14%*temps 95 (arbitraire)
...
%Permet de lire la base de données wagon et loco pour créer le fichier config
if (estLoco)
...
%Permet de récupérer les parametres présent dans le fichier bremsen.dat
%sous le dossier dat
